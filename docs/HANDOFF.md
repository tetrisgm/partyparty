...
Live-test triage (set still running; nothing live was touched). Symptom: episodic audio cuts on all three phones (Laura/LJ worst), phones drifted to 4.0s/4.8s/16.7s against the ~1s room target while capture, throughput and buffers stayed healthy between episodes. Root cause found in web/listener.html considerNativeOutlier: OUTLIER_MAX_REATTACHES=2 was a per-session LIFETIME cap - after two corrections the aligner went permanently dead and every later stall added unbounded drift (LJ latency pinned at 3.95s for 40s while visible; the counter only reset on full transport teardown). Owner rejected my first replacement (30s->8min escalating cooldown) as built-in delay tolerance - the product wants NO tolerated excess delay. Shipped contract: every confirmed drift (>=0.75s beyond target, 3 beats, visible) gets a fresh attachment, forever; cooldown is only the mechanical land-and-remeasure minimum (15s), stretches to at most 60s only while corrections fail to stick, and resets to 15s the moment one lands (the on-target branch restores credit). Contract-test pins updated (OUTLIER_MAX_REATTACHES must NOT exist; 15000/60000 + credit-restore line pinned) and the AGENTS.md playback contract now states the no-tolerance rule with the dated 16.7s evidence. npm web suites pass. Still open (post-set): the hiccup SOURCE itself (episodic stalls; simultaneity across phones will discriminate source-side discontinuity vs per-phone Wi-Fi - a 2s-cadence roster watcher is recording to scratchpad/set-watch.csv until the set ends), the 1Hz loopback HLS session churn that floods the 60-line status log ring (observability blinded to ~28s), and rebuilding/installing so the room serves this listener fix.

Workshop checkpoint: `1785886077016-7d86f611` (product).

## Next concrete step

Verify + finish through Workshop now (files only). After the set ends: rebuild-install the app, analyze scratchpad/set-watch.csv for the stall signature, then hunt the hiccup source and the log-ring churn.
