...
Owner rapid round, all verified in the running app (his own screenshots confirmed most of it live): (1) Go live sits on the transport's center line via three equal-wing flex zones (gocapture and goend both flex 1 1 0, switch auto between; gear moved to the right wing) with 🕺 in the go-state label. (2) Confetti: 150-piece canvas burst in brand colors on the idle-to-live transition, guarded by statusSeen so a console reopened mid-set never fires, prefers-reduced-motion respected, canvas removes itself. (3) Mac post reactions with the guest palette ❤️🔥😂🎉🪩 after Reply (owner ordered Reply first), posting cid dj to the existing /api/post-reaction (increment-only server semantics; listener page already had the UI - the gap was only the console); closest() on the pill so the count <b> reacts too. (4) Bar controls unified: picker select and gear at 36px on r-md, device text 13px/600 (was a 42px pill with 14/800), hero button 44. (5) What to capture -> What to broadcast. (6) AdminWindowController: titlebarAppearsTransparent + hidden title, window background switched from the STALE light-first 0.961 gray to --bg #0e0e10 - traffic lights on the console's own dark, confirmed in the owner's screenshot. (7) Page side margins clamp(32px,6vw,96px). (8) Poster: title + Shuffle/Upload lowered onto one line near the cover bottom (poster padding-bottom s-5, coveractions back to s-4); pencil is a 34px circled control matching the Shuffle button (its em sizing had collapsed to the UA button font: buttons do not inherit font-size, so 1.1em was 14px); cover X 34px to match. (9) Profile Change overlay now covers the whole circle - the base button height (32px) beats inset:0 per CSS absolute rules, explicit width/height 100% fixes it. (10) Chip fields start at the label divider and stretch to the chip edge (margin-left:auto had opened a dead middle). (11) Bandcamp removed from event.go labels, dj editor lists and wall, precedent Mixcloud/RA. Suites green throughout; scary-banner investigation pending session-log access (TCC blocks this shell).

Workshop checkpoint: `1785877190510-10b9ade1` (product).

## Next concrete step

Verify then finish through Workshop (web/dj.html, web/wall.html, internal/event/event.go, app/Sources/PartyParty/AdminWindowController.swift). Then the 20 new party cover photos (source licensed images, webp 1500x1000, eyeball each, SOURCES manifest, extend the shuffle list). Standing: owner eyeballs menu popover + bars; TestFlight 248 beta-review decision; next phone go-live confirms relay assets + ShazamKit + the confetti moment.
