(function installPlaybackSyncCandidate(create, timelineReader) {
  ...
  const sync = create({
    ...
    reliable: () => clockReliable() && clockUncertainty < 40 && (!useNative || timelineReader(media,currentTimelineOrigin()) != null),
    programTime: () => {
      if (hls?.playingDate) return hls.playingDate.getTime();
      const origin = timelineReader(media, currentTimelineOrigin());
      return origin == null ? null : origin + media.currentTime * 1000;
    },
  ...
  beginAudible = reason => { const changed = begin(reason); if (changed) sync.start(); return changed; };
  ...
  function start() {
    serial++;
    playing = false; correction = false; joining = true;
  ...
  function repair(initial = false) {
    ...
    const pdt = programTime();
    const jump = (now() - target()*1000 - pdt) / 1000;
    if (Number.isFinite(jump) && (jump > 0.03 || (initial && jump < -0.03))) {
      ...
      if (available) {
        ...
        event('position-align', { position, initial, jump });
      }
    }
  }
  function stop() {
  ...
  function tick() {
    ...
    if (e == null) {
      rate(1);
      if (phase !== 'playing' && monotonic() - began > 2000) release('clock-unavailable');
      return;
    }
    if (phase !== 'playing' && monotonic() - began > 8000) { release('alignment-timeout'); return; }
    ...
    if (phase === 'holding') {
      if (monotonic()-lastRepair < 500) return;
      ...
      if (Math.abs(e) > .04 && attempts < 4 && (e > 0 || joining)) { repair(joining); return; }
