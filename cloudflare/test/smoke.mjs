...
test("landing, legal pages, version, and signed standalone release files are served", async () => {
  ...
  await env.DL.put("standalone/partyparty-124.01-203.zip", new Uint8Array([80, 75, 3, 4]));
  ...
  const immutable = await worker.fetch(new Request("https://partyparty.party/downloads/partyparty-124.01-203.zip"), env);
