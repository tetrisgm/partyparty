...
test("landing, legal pages, version, and signed standalone release files are served", async () => {
  ...
  await env.DL.put("standalone/partyparty-123.92-194.zip", new Uint8Array([80, 75, 3, 4]));
  ...
  const immutable = await worker.fetch(new Request("https://partyparty.party/downloads/partyparty-123.92-194.zip"), env);
