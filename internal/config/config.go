...
func Parse() Config {
	...
	flag.IntVar(&c.SegCount, "seg-count", envInt("PARTYPARTY_SEG_COUNT", 32), "LL-HLS segments kept in the playlist (32x1s = the same 32s recovery window as the old 16x2s shape: a stalled guest never falls off the back before the drift watchdog re-attaches)")
