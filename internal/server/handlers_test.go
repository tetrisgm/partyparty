...
func TestLiveProxyUsesSameOriginPath(t *testing.T) {
	...
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), "#EXT-X-START:TIME-OFFSET=-2.000,PRECISE=YES") {
	...
	if !strings.Contains(compressed.Body.String(), "#EXT-X-START:TIME-OFFSET=-2.000,PRECISE=YES") {
